...
const PrizesScreen = () => {
  ...
  React.useEffect(() => {
    const init = async () => {
      const q = query(collection(db, "handshake"), orderBy("score", "desc"), limit(200));
  ...
  return (
    ...
    <TableContainer component={Paper}>
      <Table sx={{ minWidth: 100, padding: 10 }} aria-label="custom pagination table">
        <TableBody>
          {(rowsPerPage > 0
            ? players.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
            : players
          ).map((row) => (
            <TableRow key={row.etherspotAddress}>
              <TableCell align="left" colSpan={2}>
                {row.etherspotAddress}
              </TableCell>
